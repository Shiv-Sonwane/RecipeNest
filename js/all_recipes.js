...
let unsubscribeRecipes = null; // Store the Firestore snapshot listener
...
onAuthStateChanged(auth, (user) => {
    if (!user) {
        ...
        loadAllRecipes(user); // Load all recipes
    }
});

// Load and display all recipes
function loadAllRecipes(user) {
    ...
    // Query all recipes, ordered by creation date
    const q = query(collection(db, "recipes"), orderBy("createdAt", "desc"));
    unsubscribeRecipes = onSnapshot(q, (snap) => {
        console.log("Snapshot received, docs count:", snap.size);
        ...
        snap.forEach(async (docSnap) => {
            ...
            console.log("Processing recipe:", docSnap.id, data.title, "by user:", data.userId);
            ...
            const bookmarkSnap = await getDoc(bookmarkRef).catch((error) => {
            ...
            card.innerHTML = `
                ...
                <div>
                    <button class="bookmark-btn" data-id="${docSnap.id}">${isBookmarked ? "Unbookmark" : "Bookmark"}</button>
                </div>
                <div class="comments">
                    <h4>Comments</h4>
            ...
            card.querySelector(".bookmark-btn").addEventListener("click", async () => {
                ...
                try {
                    ...
                    const bookmarkCard = allList.querySelector(`.recipe-card[data-id="${recipeId}"]`);
                    const bookmarkBtn = bookmarkCard.querySelector(".bookmark-btn");
                    
                    // Re-check bookmark state
                    const bookmarkSnap = await getDoc(bookmarkRef);
                    const isCurrentlyBookmarked = bookmarkSnap.exists();
                    ...
                    if (isCurrentlyBookmarked) {
                        ...
                        await setDoc(bookmarkRef, { recipeId, createdAt: serverTimestamp() });
                        console.log("Bookmark added for recipe:", recipeId);
            ...
            card.querySelector(".like-btn").addEventListener("click", async () => {
                ...
                try {
                    ...
                    if (isLiked) {
                        ...
                        await updateDoc(recipeRef, { likes: recipe.likes - 1 });
                        console.log("Like removed for recipe:", recipeId);
                        likeBtn.textContent = "Like";
                        likeCountSpan.textContent = recipe.likes - 1;
                    } else {
                        ...
                        likeCountSpan.textContent = (recipe.likes || 0) + 1;
                    }
                } catch (error) {
        ...
        allList.innerHTML = "<p>Error loading recipes. Please try again.</p>";
    });
}
